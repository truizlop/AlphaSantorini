...
extension MCTSNode {
    func addDirichletNoise<R: RandomNumberGenerator>(
        ...
        let sample = sampleDirichlet(alpha: alphas, rng: &rng)
...
func sampleDirichlet<R: RandomNumberGenerator>(alpha: [Float], rng: inout R) -> [Float] {
    let gamma = alpha.map { a in
        sampleGamma(shape: a, rng: &rng)
    }
    let sum = gamma.reduce(0, +)
    return gamma.map { g in g / sum }
}

func sampleGamma<R: RandomNumberGenerator>(shape: Float, rng: inout R) -> Float {
    precondition(shape > 0, "Gamma shape must be > 0")
    if shape < 1 {
        // Gamma(k) = Gamma(k + 1) * U^(1/k) for k in (0, 1)
        let u = Float.random(in: Float.leastNonzeroMagnitude ..< 1, using: &rng)
...
func sampleStandardNormal<R: RandomNumberGenerator>(rng: inout R) -> Float {
